...
- A TPS/MSPT line in the vanilla player-list header.
...
The release jar embeds only the four small Fabric API modules it uses, so a separate Fabric API install is not required. It also works when the full Fabric API is already installed.

## Install

1. Stop the server.
2. Put `MidgetControl-1.0.0.jar` in the server's `mods` folder.
3. Start the server once.
4. Edit `config/midgetcontrol.properties` if wanted.
5. Run `/midgetcontrol reload` as the server owner, or restart the server.

Player history is stored separately per world in `world/data/midgetcontrol-players.json`.
...
## Tab-header compatibility

Minecraft exposes one shared tab-list header. If another mod also owns that header, the most recent update wins. Disable `tab-tps.enabled` in one of the mods if they compete; all other MidgetControl features continue to work.
